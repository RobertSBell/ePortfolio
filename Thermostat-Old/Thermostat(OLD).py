...
class ManagedDisplay():
    ...
    def __init__(self):
        ...
        ##self.lcd.create_char(1, doubleT)
        ##self.lcd.create_char(2, pColon)
...
class TemperatureMachine(StateMachine):
    ...
    ##
    ## Default temperature setPoint is 72 degrees Fahrenheit3
    ##
    setPoint = 72
    ...
    def updateLights(self):
        ...
        if self.current_state.id == "heat":
            # Pulse when less than specified temp
            if temp < self.setPoint:
                redLight.pulse()
            # Solid when equal or greater than
            else:
                redLight.on()

        elif self.current_state.id == "cool":
            # Pulse when greater than specified temp
            if temp > self.setPoint:
                blueLight.pulse()
            # Solid when equal or less than
            else:
                blueLight.on()
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                # \x00 calls on custom degree(°) character
                lcd_line_2 = f"CurrTemp: {self.getFahrenheit():0.1f}\x00F"
                ##
                ...
                if(self.current_state.id == "off"):
                    lcd_line_2 = f"{self.current_state.id.capitalize()}  STemp: {self.setPoint}\x00F"
                else:
